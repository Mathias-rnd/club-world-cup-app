...
from scrape_quarter_finals import (
    scrape_with_requests as scrape_quarter_finals_with_requests,
)
...
POINTS = {"1/8": 2, "1/4": 4, "1/2": 8, "Final": 16, "Winner": 32, "BestStriker": 20}
...
def compute_score(bet, results):
    ...
    for round_name in ["1/8", "1/4", "1/2", "Final"]:
        actual_teams = {
            str(team).strip().lower() for team in results.get(round_name, [])
        }
        bet_teams_str = bet.get(round_name, "")
        bet_teams = (
            {team.strip().lower() for team in bet_teams_str.split(";")}
            if bet_teams_str
            else set()
        )
        ...
        if round_name == "1/4":
            all_1_4_teams = {
                ...
                for team in results.get("1/4", []) + results.get("1/4_losers", [])
            }
            ...
            all_1_2_teams = {
                str(team).strip().lower()
                for team in results.get("1/2", []) + results.get("1/2_losers", [])
            }
    ...
    if (
        bet.get("Winner", "").strip().lower()
        == results.get("Winner", "").strip().lower()
    ):
        score += POINTS["Winner"]
    if (
        bet.get("BestStriker", "").strip().lower()
        == results.get("BestStriker", "").strip().lower()
    ):
...
def compute_joker_points(joker_prediction, results):
    ...
    try:
        ...
        predicted_winner = (
            predicted_team1 if predicted_score1 > predicted_score2 else predicted_team2
        )
        ...
        for match in all_matches:
            ...
            if (
                team_names_match(match["team1"], predicted_team1)
                and team_names_match(match["team2"], predicted_team2)
            ) or (
                team_names_match(match["team1"], predicted_team2)
                and team_names_match(match["team2"], predicted_team1)
            ):
                if match["status"] == "finished" and match["score"]:
                    ...
                    if len(actual_score_parts) == 2:
                        ...
                        actual_winner = (
                            match["team1"]
                            if actual_score1 > actual_score2
                            else match["team2"]
                        )

                        # Calculate points
                        if (
                            predicted_score1 == actual_score1
                            and predicted_score2 == actual_score2
                        ):
...
def get_leaderboard():
    ...
    leaderboard = [
        {"name": bet["Name"], "score": compute_score(bet, results)} for bet in bets
    ]
    ...
    for round_key in ["1/8", "1/4", "1/2", "Final"]:
        ...
        if teams and teams[0]:
            ...
            for team in teams:
                ...
                if round_key == "1/4":
                    all_1_4_teams = results_data.get("1/4", []) + results_data.get(
                        "1/4_losers", []
                    )
                    ...
                    all_1_2_teams = results_data.get("1/2", []) + results_data.get(
                        "1/2_losers", []
                    )
                    ...
                    if in_list(team, eliminated_teams):
                        ...
                        final_not_played = not winner or any(
                            t in ["N.N.", "", None] for t in final_teams
                        )
                        if final_not_played:
                            ...
                            if in_list(team, final_teams):
                                ...
                                from scrape_semi_finals import (
                                    scrape_with_requests as scrape_semi_finals_with_requests,
                                )
                                ...
                                for m in matches_1_2:
                                    if (
                                        team_names_match(team, m["team1"])
                                        or team_names_match(team, m["team2"])
                                    ) and m["status"] == "live":
                                        score = m.get("score", "")
    ...
    for i in range(1, 4):
        ...
        if joker_bet and joker_bet.strip():
            ...
            if joker_points == 4:
                ...
                status = "pending"
            jokers_1_8.append(
                {"team": joker_bet, "status": status, "points": joker_points}
            )
        else:
...
def refresh_quarters_data():
    ...
    try:
        ...
        if not matches:
            ...
            return jsonify(
                {"success": False, "message": "Scraping failed to return any data."}
            ), 500
        ...
        return jsonify(
            {"success": True, "message": "Semi-finals data refreshed successfully!"}
        )
...
def refresh_top_scorers():
    try:
        ...
        if not os.path.exists("scrape_top_scorers.py"):
            ...
            return jsonify(
                {"success": False, "message": "scrape_top_scorers.py not found"}
            ), 500

        # Try to run the script
        result = subprocess.run(
            ["python3", "scrape_top_scorers.py"],
            capture_output=True,
            text=True,
            check=True,
        )
        ...
        if os.path.exists("top_scorers.json"):
            ...
            return jsonify(
                {"success": False, "message": "top_scorers.json was not created"}
            ), 500
        ...
        return jsonify(
            {"success": False, "message": f"Script execution failed: {e.stderr}"}
        ), 500
    except Exception as e:
...
def update_joker():
    try:
        ...
        return jsonify(
            {"success": True, "message": f"Joker prediction updated for {player_name}"}
        )

    except Exception as e:
        return jsonify(
            {"success": False, "message": f"Error updating joker: {str(e)}"}
        ), 500
...
def get_bets_for_admin():
    try:
        ...
        return jsonify(
            {"success": False, "message": f"Error loading bets: {str(e)}"}
        ), 500
